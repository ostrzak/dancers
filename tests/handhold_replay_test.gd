...
func _run() -> void:
	...
	for index in 2:
		...
		if extreme:
			dancer.set_weight_kg(100.0 if index == 0 else 50.0)
		if reference:
	...
	print("CAPTURE REPLAY reference_mass=%s gap=%.4f tail_spin_step_rms=%.4f final_spin=%s holds=%d" % [
		reference, peak_gap, tail_rms, previous_spin, scene.hand_connection.get_active_connection_count()])
	var passed := peak_gap <= 36.1 and tail_rms < 0.15 and scene.hand_connection.get_active_connection_count() == 1
