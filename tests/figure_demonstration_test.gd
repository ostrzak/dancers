...
func _run() -> void:
	...
	# Action checks seek by choreography phase so longer loops retain the same poses.
	# Check the intended actions, not only contact geometry.
	for selection in [[5, 0], [5, 1], [6, 0]]:
		...
		for frame in range(721):
			demo.elapsed = frame / 720.0 * demo.get_figure().duration
			demo._update_pose()
			fixed_throughout = fixed_throughout and demo.ghost_a.position.is_equal_approx(fixed_position)
		_expect(fixed_throughout, "%s: gentleman stays in place" % demo.get_figure().title)
		_expect(is_equal_approx(demo.ghost_b.rotation - initial_turn, -TAU if selection[1] == 1 else TAU),
			"%s: lady completes a full turn" % demo.get_figure().title)
	for variant in [0, 1]:
	...
	demo.elapsed = 3.0 * demo.get_figure().duration / 6.0
	demo._update_pose()
	...
	demo.elapsed = demo.get_figure().duration
	demo._update_pose()
	...
	_expect(demo.FIGURES.size() == 21, "connected turn variants share one of twenty-one figure entries")
	...
	demo.elapsed = 2.7 * demo.get_figure().duration / 6.0
	demo._update_pose()
	...
	demo.elapsed = 4.1 * demo.get_figure().duration / 6.0
	demo._update_pose()
	...
	var turn_end := demo.get_figure().sample(demo.get_figure().duration)
	...
	var first_side := demo.get_figure().sample(1.0 * demo.get_figure().duration / 6.0)
	var other_side := demo.get_figure().sample(3.0 * demo.get_figure().duration / 6.0)
	...
	var closed := demo.get_figure().sample(1.5 * demo.get_figure().duration / 6.0)
	var opened := demo.get_figure().sample(3.0 * demo.get_figure().duration / 6.0)
	...
	demo.elapsed = 4.5 * demo.get_figure().duration / 6.0
	demo._update_pose()
	...
	demo.elapsed = demo.get_figure().duration + demo.END_PAUSE + 2.0 * demo.FADE_TIME - 0.01
	demo._process(0.04)
