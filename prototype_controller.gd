...
func _apply_arm_pose_inputs(left_input: Vector2, right_input: Vector2, delta: float) -> void:
	if get_tree().paused:
		return
	# Consent is always per axis, including while apart. Agreed changes apply
	# the same bounded delta to both dancers in their anatomical coordinates.
	var agreed := Vector2(
		_agreed_pose_axis(left_input.x, right_input.x),
	...
	var spread_step := agreed.x * step
	var sweep_step := agreed.y * step
	for dancer in [left_dancer, right_dancer]:
		spread_step = clampf(spread_step,
			dancer.minimum_abduction_degrees - dancer.extended_abduction_degrees,
			dancer.maximum_abduction_degrees - dancer.extended_abduction_degrees)
	...
	for dancer in [left_dancer, right_dancer]:
		dancer.extended_abduction_degrees += spread_step
